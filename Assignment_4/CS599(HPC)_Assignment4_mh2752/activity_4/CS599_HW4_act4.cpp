...
int main(int argc, char **argv) {
  ...
  // ------------------------------ Write code here -----------------------------------------------
    ...
    if(my_rank == 0)
    {
      ...
      fprintf(stdout,"\n\t[Process %d] nprocs = %d, Total time = %lf, construction time = %lf, search time = %lf, global sum = %u\n",my_rank,nprocs,max_total,max_construction,max_search,global_positive_hits);
      fflush(stdout);
